...
void Engine::ItemSpawner::Update(float dt, std::vector<std::shared_ptr<IGamePart>>& gameParts)
{
	...
	{
		_gameParts.push_back(std::make_shared<Item>(_player, gunAmmo, sf::Vector2f{ 500, 500 }, "ammoTexture", _data, _gameParts));
		_gameParts.push_back(std::make_shared<Item>(_player, health, sf::Vector2f{ 700, 500 }, "hpTexture", _data, _gameParts));
